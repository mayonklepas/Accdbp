...
public class UserAppController {
    ...
    @PostMapping("")
    public ResponseEntity<?> create(@RequestBody UserApp payload) {
        ServiceResponseData<?> result = srv.create(payload);
        return ResponseEntity.status(result.getStatusCode()).body(result);
    }

    @PutMapping("/id/{id}")
    public ResponseEntity<?> update(@RequestBody UserApp payload, @PathVariable("id") long id) {
    ...
    @PutMapping("/id/{id}/update-password")
    public ResponseEntity<?> enabledCustomer(@RequestParam String newPassword,@PathVariable("id") long id) {
        ServiceResponseData<?> result = srv.updatePassword(newPassword, id);
        return ResponseEntity.status(result.getStatusCode()).body(result);
    }
